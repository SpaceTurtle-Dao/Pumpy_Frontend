...
<div class="py-24 w-full flex flex-col justify-center items-center min-h-screen">
	<div class="w-full max-w-md">
		<div class="flex flex-row justify-between pb-4">
			<Button
				variant="outline"
				class="h-10 w-10 border-none rounded-full hover:border-input text-blue-300 bg-primary hover:bg-blue-800/70 hover:text-blue-100 text-xs transition-all duration-300 ease-in-out"
				size="icon"
			>
		...
		<Card.Root
			class="bg-blue-900/30 backdrop-blur-md border-blue-700/50 rounded-2xl px-6 py-8 shadow-xl border"
		>
			<div class="mb-6">
				...
				<div class="flex items-center bg-blue-800/30 rounded-xl pl-4 py-4 space-x-2 w-full">
					{#if tokens !== undefined && tokens.length > 0}
						<div
							class="group space-x-2.5 border border-transparent relative flex items-center bg-blue-900/50 hover:bg-blue-800/70 hover:border-blue-400 hover:shadow-lg hover:shadow-blue-500/30 text-blue-200 rounded-lg px-3 py-2 transition-all duration-300 ease-in-out"
						>
							...
							>
								{#each tokens as token}
									<option value={token.ticker} selected={token.ticker === _fromCurrency.ticker}
										>{token.ticker}</option
									>
								{/each}
							</select>
				...
				<div class="flex items-center bg-blue-800/30 rounded-xl pl-4 py-4 space-x-2 w-full">
					{#if tokens !== undefined && tokens.length > 0}
						...
						>
							...
							>
								{#each tokens as token}
									<option value={token.ticker} selected={token.ticker === _toCurrency.ticker}
										>{token.ticker}</option
									>
								{/each}
							</select>
			...
			<div class="text-center">
				...
				{#if showRateInfo}
					<p transition:fade={{ duration: 300 }} class="mt-2 text-blue-200">
						1 {_fromCurrency.ticker} = {exchangeRate}
						{_toCurrency.ticker}
					</p>
				{/if}
			</div>
